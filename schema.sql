...
create policy "Classes modifiable by admins"
  on public.classes for all
  using (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'))
  with check (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));

create policy "Reservations readable by members"
  on public.reservations for select
  using (auth.role() = 'authenticated');

create policy "Reservations insertable by owner"
  on public.reservations for insert
  with check (auth.uid() = user_id);

create policy "Reservations deletable by owner"
  on public.reservations for delete
  using (auth.uid() = user_id or exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));

create policy "Reservations updatable by admins"
...
create policy "Notifications insertable by server"
...
-- Functions to reserve/cancel and keep waitlist consistent
